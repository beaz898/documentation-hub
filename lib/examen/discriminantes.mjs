 ...
 *
 * ⚠️ LO QUE ESTE VERIFICADOR **NO** CAZA HOY, medido el 25/09/2026 sobre los
 * cinco documentos nuevos: **una frase partida por un SALTO DE LÍNEA DEL
 * DOCUMENTO**. `fragmentosConLaFrase` compara con `includes`, exacto, y el
 * producto decide lo mismo con `normalize()` (`lib/analysis/normalize.ts:37-43`),
 * que COLAPSA los espacios. Son dos implementaciones del mismo criterio y ya
 * divergen: los discriminantes de los falsos 1 y 5, transcritos de F-22, dieron
 * `AUSENTE` estando las dos frases en sus documentos, maquetadas a 70 columnas.
 * Un `NO_MEDIBLE` falso, con un mensaje que manda a buscar donde no está.
 * El detalle y las tres opciones, en `corpus-pruebas/SIEMBRA_falsos_de_agosto.md`
 * (fragilidad 3). No se arregla desde aquí: la cura toca `normalize.ts`, que
 * lleva escrito un invariante de co-locación, y eso lo decide el arquitecto.
 *
...
function costuraDe(frase, fragmentos) {
  for (let i = 0; i < fragmentos.length - 1; i++) {
    const a = fragmentos[i].text;
    const b = fragmentos[i + 1].text;
    // El corte puede caer en cualquier punto interior de la frase.
    for (let corte = 1; corte < frase.length; corte++) {
      const cabeza = frase.slice(0, corte);
      const cola = frase.slice(corte);
      if (a.endsWith(cabeza) && b.startsWith(cola)) {
        return {
...
/** Los fragmentos donde la frase aparece ENTERA. */
function fragmentosConLaFrase(frase, fragmentos) {
  return fragmentos.filter(f => f.text.includes(frase)).map(f => f.chunkIndex);
}
