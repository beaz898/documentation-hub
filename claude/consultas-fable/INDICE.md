...
| **[F-114](F-114.md)** | 21/09/2026 | El suelo medido, y dónde vive el termómetro. **AUTORIZA la retirada de los dos umbrales y del alias**: suelo **0,696** (n=2.031, topK=1000 sobre fondo 680) y cero fragmentos bajo 0,50 y 0,45 en los tres extremos. El termómetro va al jsonb `analysis` con cuatro condiciones; canario de DOS parejas nunca indexadas, medido en cada censo; sello del modelo desde `embeddings.ts`. Orden revisado: rama atómica → termómetro → constantes → erratas | **vigente** · PAR COMPLETO · ⚠️ **PREDICCIÓN DE FABLE FALLADA Y CONTADA POR SU AUTOR**: predijo «mínimo ≥ 0,70» y salió 0,696 — falla por 0,004; su explicación literal: «me anclé en la cifra redonda de la ficha del fabricante, que describía un rango y no medía nada». ⚠️ Y **RETIRA su propia condición de F-113** de meter el termómetro en `COUNTER_CATALOGUE` · pendiente: la MATRIZ COMPLETA (680 consultas) con su predicción 0,62-0,696 y su condición de parada | El suelo baja cuando el fondo es más variado (hallazgo del arquitecto, no de Fable) |
