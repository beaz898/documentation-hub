...
| **El lote alcanza a DOS documentos, y es por una línea — B.200** | `plan-de-reindexado.ts:90` comprueba `via_no_construida` **antes** que la viabilidad de `retrocear`, así que 36 documentos reciben un 501 por una reparación que funcionaría. ⚠️ **Que el lote alcance a dos es un hecho sobre ESA LÍNEA, no sobre el corpus** — y se escribe así para que nadie lo lea al revés más adelante. Invertir las dos comprobaciones repararía los 28 de prosa desde casa; los 8 `.xlsx` seguirían rechazándose por la guarda de B.191, que es lo correcto. **No se toca hoy**: puede ser deliberado —preferir la reparación completa a la disponible es B.195, que se cerró decidiendo lo contrario— y hace falta mirarlo antes de moverlo |
