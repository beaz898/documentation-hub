...
export default function LandingPage() {
  ...
  const PLANS = [
    { name: 'Free',       price: '0',   credits: '50',     users: 1,    desc: t('plans.free'),       cta: t('cta.startFree') },
    { name: 'Starter',    price: '59',  credits: '400',    users: 3,    desc: t('plans.starter'),    cta: t('cta.start') },
    { name: 'Pro',        price: '149', credits: '1.500',  users: 5,    desc: t('plans.pro'),        cta: t('cta.start'), popular: true },
    { name: 'Business',   price: '349', credits: '4.000',  users: 15,   desc: t('plans.business'),   cta: t('cta.start') },
    { name: 'Business+',  price: '599', credits: '10.000', users: null, desc: t('plans.businessPlus'), cta: t('cta.contact') },
  ];
    ...
    <div className="landing" style={{ background: '#fafaf9', color: '#1a1a1a', minHeight: '100vh' }}>
          ...
          <div className="hero-layout" style={{ display: 'flex', alignItems: 'center', gap: 48 }}>
            <div className="hero-text" style={{ flex: '0 0 46%', display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
              ...
              <h1 style={{ fontSize: 'clamp(34px, 4.5vw, 54px)', lineHeight: 1.12, letterSpacing: -1, marginBottom: 18 }}>
                {t('hero')}<br /><span style={{ color: BRAND }}>{t('heroHighlight')}</span>
              </h1>
              ...
              <p style={{ fontSize: 12, color: '#999', marginTop: 14 }}>{t('heroNoCredit')}</p>
            </div>
      ...
      <section style={{ paddingBottom: 60 }}>
        <div className="landing-section" style={{ textAlign: 'center' }}>
          <p style={{ fontSize: 12, color: '#bbb', letterSpacing: 0.5, textTransform: 'uppercase', fontWeight: 500 }}>{t('trustBuiltWith')}</p>
      ...
      <section style={{ paddingTop: 60, paddingBottom: 80 }}>
        <div className="landing-section">
          ...
          <div className="features-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
            {[
              { color: '#fef2f2', icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#dc2626" strokeWidth="1.8"><circle cx="11" cy="11" r="8" /><line x1="21" y1="21" x2="16.65" y2="16.65" /><line x1="8" y1="11" x2="14" y2="11" /></svg>, title: t('problem1Title'), text: t('problem1Text') },
              { color: '#fefce8', icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#ca8a04" strokeWidth="1.8"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z" /><line x1="12" y1="9" x2="12" y2="13" /><line x1="12" y1="17" x2="12.01" y2="17" /></svg>, title: t('problem2Title'), text: t('problem2Text') },
              { color: '#fdf2f8', icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#db2777" strokeWidth="1.8"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg>, title: t('problem3Title'), text: t('problem3Text') },
            ].map((item, i) => (
        ...
        <div className="landing-section">
          ...
          <div className="showcase-row" style={{ display: 'flex', gap: 48, alignItems: 'center', marginBottom: 80 }}>
            <div className="showcase-text" style={{ flex: '0 0 38%', display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
              <span style={{ fontSize: 11, fontWeight: 700, color: BRAND, textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 12 }}>{t('showcase1Label')}</span>
              <h3 style={{ fontSize: 24, fontWeight: 700, lineHeight: 1.25, marginBottom: 12, fontFamily: "'DM Sans',sans-serif" }}>{t('showcase1Title')}</h3>
              <p style={{ fontSize: 14, lineHeight: 1.7, color: '#666', marginBottom: 16 }}>{t('showcase1Desc')}</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {[t('showcase1F1'), t('showcase1F2'), t('showcase1F3')].map((feat, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: '#555' }}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={BRAND} strokeWidth="2.5"><polyline points="20 6 9 17 4 12" /></svg>{feat}
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="showcase-row showcase-row-reverse" style={{ display: 'flex', flexDirection: 'row-reverse', gap: 48, alignItems: 'center', marginBottom: 80 }}>
            <div className="showcase-text" style={{ flex: '0 0 38%', display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
              <span style={{ fontSize: 11, fontWeight: 700, color: '#f59e0b', textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 12 }}>{t('showcase2Label')}</span>
              <h3 style={{ fontSize: 24, fontWeight: 700, lineHeight: 1.25, marginBottom: 12, fontFamily: "'DM Sans',sans-serif" }}>{t('showcase2Title')}</h3>
              <p style={{ fontSize: 14, lineHeight: 1.7, color: '#666', marginBottom: 16 }}>{t('showcase2Desc')}</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {[t('showcase2F1'), t('showcase2F2'), t('showcase2F3')].map((feat, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: '#555' }}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#f59e0b" strokeWidth="2.5"><polyline points="20 6 9 17 4 12" /></svg>{feat}
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="showcase-row" style={{ display: 'flex', gap: 48, alignItems: 'center' }}>
            <div className="showcase-text" style={{ flex: '0 0 38%', display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
              <span style={{ fontSize: 11, fontWeight: 700, color: '#10b981', textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 12 }}>{t('showcase3Label')}</span>
              <h3 style={{ fontSize: 24, fontWeight: 700, lineHeight: 1.25, marginBottom: 12, fontFamily: "'DM Sans',sans-serif" }}>{t('showcase3Title')}</h3>
              <p style={{ fontSize: 14, lineHeight: 1.7, color: '#666', marginBottom: 16 }}>{t('showcase3Desc')}</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {[t('showcase3F1'), t('showcase3F2'), t('showcase3F3')].map((feat, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: '#555' }}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#10b981" strokeWidth="2.5"><polyline points="20 6 9 17 4 12" /></svg>{feat}
                  </div>
                ))}
              </div>
            </div>
                ...
                <div style={{ display: 'flex', height: 260 }}>
                  ...
                  <div style={{ width: 200, padding: 12, background: '#fafaf9', display: 'flex', flexDirection: 'column', gap: 8 }}>
                    ...
                    <div className="m3-apply">
                      <div style={{ padding: '6px 10px', borderRadius: 6, background: '#10b981', color: '#fff', fontSize: 10, fontWeight: 600, textAlign: 'center' }}>✓ Aplicar corrección</div>
                    </div>
                    <div className="m3-resolved" style={{ padding: '6px 10px', borderRadius: 6, background: '#dcfce7', border: '1px solid #bbf7d0', fontSize: 10, color: '#16a34a', fontWeight: 600, textAlign: 'center' }}>✓ Problema resuelto</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="landing-section">
          <div style={{ padding: 'clamp(36px,4vw,56px)', borderRadius: 20, background: 'linear-gradient(135deg, #f0f9ff 0%, #eff6ff 50%, #f5f3ff 100%)', border: '1px solid rgba(37,99,235,.08)' }}>
            <div className="teams-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 48, alignItems: 'center' }}>
              <div>
                <span style={{ fontSize: 11, fontWeight: 700, color: BRAND, textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 12, display: 'block' }}>{t('teamsLabel')}</span>
                <h2 style={{ fontSize: 'clamp(22px,3vw,32px)', lineHeight: 1.25, letterSpacing: -0.3, marginBottom: 14 }}>{t('teamsTitle')}</h2>
                <p style={{ fontSize: 14, lineHeight: 1.7, color: '#666', marginBottom: 20 }}>{t('teamsDesc')}</p>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, padding: '0 20px' }}>
                {[
                  { name: 'Ana García',    role: t('teamsRoleAdmin'),  queries: 142, color: BRAND },
                  { name: 'Carlos López',  role: t('teamsRoleMember'), queries: 87,  color: '#10b981' },
                  { name: 'María Torres',  role: t('teamsRoleMember'), queries: 63,  color: '#f59e0b' },
                ].map((member, i) => (
                  <div key={i} style={{ padding: '14px 18px', borderRadius: 12, background: '#fff', border: '1px solid rgba(0,0,0,.06)', display: 'flex', alignItems: 'center', gap: 12, boxShadow: '0 2px 8px rgba(0,0,0,.03)' }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      ...
                      <div style={{ fontSize: 10, color: '#bbb' }}>{t('teamsMemberQueries')}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="diff-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 48, alignItems: 'center', position: 'relative' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <div style={{ padding: '22px 24px', borderRadius: 14, background: `${BRAND}22`, border: `1px solid ${BRAND}44` }}>
                  ...
                  <div style={{ marginTop: 14, display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {[t('diffDoclityF1'), t('diffDoclityF2'), t('diffDoclityF3')].map((feat, i) => (
      ...
      <section style={{ paddingTop: 60, paddingBottom: 80 }}>
        <div className="landing-section">
          ...
          <h2 style={{ fontSize: 'clamp(26px,4vw,40px)', textAlign: 'center', lineHeight: 1.2, letterSpacing: -0.5, marginBottom: 12 }}>{t('plansTitle')}</h2>
          <p style={{ fontSize: 14, color: '#999', textAlign: 'center', marginBottom: 48 }}>{t('plansDisclaimer')}</p>
          <div className="plans-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 14, maxWidth: 960, margin: '0 auto' }}>
            {PLANS.map((plan, i) => (
              <div key={i} className={`plan-card ${plan.popular ? 'plan-popular' : ''}`}>
                {plan.popular && (
                  <span style={{ position: 'absolute', top: -11, left: '50%', transform: 'translateX(-50%)', padding: '4px 16px', borderRadius: 12, background: BRAND, color: '#fff', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, boxShadow: `0 2px 8px ${BRAND}44` }}>
                    {t('plansMostPopular')}
                  </span>
                )}
                ...
                <div style={{ fontSize: 13, color: '#666', marginBottom: 20, flex: 1 }}>
                  <p style={{ marginBottom: 6, display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    {plan.credits} {t('plansCreditsPerMonth')}
                  </p>
                  <p style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={BRAND} strokeWidth="2.5"><polyline points="20 6 9 17 4 12" /></svg>
                    {plan.users !== null ? t('plansUsersLimit', { count: plan.users }) : t('plansUsersUnlimited')}
                  </p>
                </div>
                <button
                  onClick={handlePlanCTA}
                  style={{ width: '100%', padding: '10px', borderRadius: 8, border: 'none', background: plan.popular ? BRAND : '#f0f0ee', color: plan.popular ? '#fff' : '#1a1a1a', fontSize: 13, fontWeight: 600, cursor: 'pointer', transition: 'all .15s', boxShadow: plan.popular ? `0 2px 8px ${BRAND}33` : 'none' }}
      ...
      <section style={{ paddingTop: 40, paddingBottom: 80 }}>
        <div className="landing-section" style={{ maxWidth: 700, textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: 'clamp(22px,3vw,32px)', lineHeight: 1.3, letterSpacing: -0.3, marginBottom: 16 }}>{t('securityTitle')}</h2>
          <p style={{ fontSize: 15, lineHeight: 1.7, color: '#666', marginBottom: 28 }}>{t('securityDesc')}</p>
          <div className="security-badges" style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap' }}>
            {[
              { icon: '🔒', text: t('securityB1') },
              { icon: '🏢', text: t('securityB2') },
              { icon: '🇪🇺', text: t('securityB3') },
              { icon: '🚫', text: t('securityB4') },
            ].map((item, i) => (
      ...
      <section style={{ paddingTop: 40, paddingBottom: 80 }}>
        <div className="landing-section" style={{ maxWidth: 640 }}>
          <h2 style={{ fontSize: 'clamp(26px,4vw,36px)', textAlign: 'center', lineHeight: 1.2, letterSpacing: -0.5, marginBottom: 40 }}>{t('faqTitle')}</h2>
          <div>
            {FAQS.map((faq, i) => (
              <div key={i} className="faq-item" onClick={() => setOpenFaq(openFaq === i ? null : i)}>
                <div className="faq-q">
                  <span>{faq.q}</span>
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#999" strokeWidth="2" style={{ transform: openFaq === i ? 'rotate(180deg)' : 'none', transition: 'transform .25s', flexShrink: 0 }}><polyline points="6 9 12 15 18 9" /></svg>
                </div>
        ...
        <div className="landing-section" style={{ textAlign: 'center' }}>
          <div style={{ padding: 'clamp(40px,5vw,64px)', borderRadius: 24, background: `linear-gradient(135deg, ${BRAND_LIGHT} 0%, #eff6ff 100%)`, border: `1px solid ${BRAND}15` }}>
            <h2 style={{ fontSize: 'clamp(28px,4.5vw,44px)', lineHeight: 1.2, letterSpacing: -0.5, marginBottom: 16 }}>
              {t('ctaFinalTitle')}<br /><span style={{ color: BRAND }}>{t('ctaFinalHighlight')}</span>
            </h2>
            <p style={{ fontSize: 16, color: '#555', marginBottom: 32, maxWidth: 480, margin: '0 auto 32px' }}>{t('ctaFinalDesc')}</p>
      ...
      <footer style={{ borderTop: '1px solid rgba(0,0,0,.06)', padding: '32px 24px' }}>
        <div className="landing-section" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap' }}>
            <a href="/legal/privacidad" style={{ fontSize: 13, color: '#999', textDecoration: 'none' }}>{t('footerPrivacy')}</a>
            <a href="/legal/terminos" style={{ fontSize: 13, color: '#999', textDecoration: 'none' }}>{t('footerTerms')}</a>
            <a href="/legal/aviso-legal" style={{ fontSize: 13, color: '#999', textDecoration: 'none' }}>{t('footerAviso')}</a>
            <a href="/legal/cookies" style={{ fontSize: 13, color: '#999', textDecoration: 'none' }}>{t('footerCookies')}</a>
          </div>
        </div>
      </footer>
    </div>
